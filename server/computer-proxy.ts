...
async function runOnSandbox(command: string, timeoutMs = 60_000) {
...
async function readSandboxFile(path: string): Promise<string | null> {
  try {
    const sandbox = await getSandbox();
...
const TOOLS = [
  {
    name: "screenshot",
    description:
      "Attempt to capture the sandbox screen (returns an image if X11/display is available). e2b sandboxes are headless by default — this may fail.",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "sandbox_exec",
    description:
      "Run a shell command on the bot's e2b sandbox (Linux, isolated environment). Returns stdout/stderr/exit code.",
...
async function call(id: unknown, name: string, args: any) {
  ...
  if (name === "write_file") {
    ...
    if (!path) return text(id, "path required", true);
    try {
      const sandbox = await getSandbox();
      await sandbox.files.write(path, content);
      return text(id, `wrote ${content.length} bytes to ${path}`);
    } catch (e) {
      return text(id, `write failed: ${(e as Error).message}`, true);
    }
  }
  if (name === "read_file") {
    const path = String(args.path ?? "");
    ...
    try {
      ...
      return text(id, `read failed: ${(e as Error).message}`, true);
    }
  }
  if (name === "list_files") {
    const path = String(args.path ?? "/");
    try {
      ...
      const listing = files.map((f: any) => `${f.type === "dir" ? "d" : "-"} ${f.name}`).join("\n");
      return text(id, listing || "(empty directory)");
    } catch (e) {
      return text(id, `list failed: ${(e as Error).message}`, true);
    }
  }
...
async function handle(msg: any) {
  ...
  if (msg.method === "tools/call") {
    try {
      ...
      return text(msg.id, `sandbox tool failed: ${(e as Error).message}`, true);
    }
  }
