...
export function normalizeToolCalls(raw: unknown): OpenAIToolCall[] {
...
export async function openaiComplete(opts: {
  ...
  for (;;) {
    ...
    while ((nl = buf.indexOf("\n")) !== -1) {
      ...
      if (Array.isArray(delta?.tool_calls)) accumulateToolCallDelta(toolAcc, delta.tool_calls);
...
export async function runOpenAITurn(opts: {
  ...
  onToolDone?: (id: string, ok: boolean) => void;
  errorPrefix: string;
  extraBody?: Record<string, unknown>;
  executeTool?: (name: string, args: unknown) => Promise<unknown>;
  listTools?: (auth: ComposioAuth) => Promise<OpenAIFunctionTool[]>;
  maxIters?: number;
  ...
  const execute =
    ...
    (async (name: string, args: unknown) => {
      if (!opts.composio) throw new Error("no Composio auth");
  ...
  for (let i = 0; i < maxIters; i++) {
    ...
    if (!result.toolCalls.length) {
      if (useTools && result.text) opts.onDelta?.(result.text);
