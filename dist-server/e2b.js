...
export function e2bConfigured(cfg) {
    return Boolean(cfg.e2b?.apiKey);
}
function getApiKey(cfg) {
    const key = cfg.e2b?.apiKey;
    if (!key) {
        throw new Error('e2b not configured — add {"e2b":{"apiKey":"e2b_…"}} to ~/.config/workspacealberta/config.json or set E2B_API_KEY');
    }
    return key;
}
/** Find the bot's existing sandbox by metadata (running or paused). */
export async function findSandbox(cfg, botId) {
...
export async function sandboxStatus(cfg, botId) {
    if (!e2bConfigured(cfg))
        return { configured: false, sandbox: null };
    try {
 ...
 */
export async function provisionSandbox(cfg, botId, botName) {
    const apiKey = getApiKey(cfg);
...
export async function joinSandbox(cfg, botId) {
    const apiKey = getApiKey(cfg);
    const info = await findSandbox(cfg, botId);
    if (!info) {
...
export async function pauseSandbox(cfg, botId) {
    const apiKey = getApiKey(cfg);
    const info = await findSandbox(cfg, botId);
    if (!info) {
...
export async function killSandbox(cfg, botId) {
    const apiKey = getApiKey(cfg);
    const info = await findSandbox(cfg, botId);
    if (!info) {
        throw new Error("no sandbox for this bot");
...
export async function execOnSandbox(cfg, botId, command) {
    const apiKey = getApiKey(cfg);
    const info = await findSandbox(cfg, botId);
    if (!info) {
...
export async function runCommand(cfg, sandboxId, command, { timeoutMs = 120_000 } = {}) {
    const apiKey = getApiKey(cfg);
...
export async function readFile(cfg, sandboxId, path) {
    const apiKey = getApiKey(cfg);
    try {
        const sandbox = await Sandbox.connect(sandboxId, { apiKey });
...
export async function screenshotSandbox(cfg, botId) {
    const apiKey = getApiKey(cfg);
    const info = await findSandbox(cfg, botId);
    if (!info) {
        throw new Error("no sandbox for this bot yet");
    ...
    if (out.stdout?.includes("no_display")) {
        throw new Error("e2b sandboxes have no graphical desktop by default — screenshot unavailable");
    }
    if (!out.stdout?.includes("captured")) {
        throw new Error(out.stderr?.slice(0, 200) || "screen capture failed on the sandbox");
    }
