...
//    EmbeddedCuaDriverHost so TCC grants attribute to WorkspaceAlberta and the
//    driver inherits them. One prompt, named WorkspaceAlberta, out of the box.
...
const HOST_BUNDLE_ID = "ca.workspacealberta.app";
...
export async function startCua() {
  ...
  const wantEmbedded =
    app.isPackaged || process.env.WA_CUA_EMBEDDED === "1" || process.env.OPENMAUSBOT_CUA_EMBEDDED === "1";
