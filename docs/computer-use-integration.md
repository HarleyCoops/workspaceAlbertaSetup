# Computer use & browser use in WorkspaceAlberta

Decision doc, 2026-08-12. How bots in WorkspaceAlberta get local computer use and
...
│     one TCC prompt, named WorkspaceAlberta     │ unix socket (private)
...
1. **Spawn from the Electron main process, never from the server/gateway
   ...
   Spawned from Electron main → the grant is WorkspaceAlberta's, users see ONE
   prompt named WorkspaceAlberta, and the bundled driver inherits it. Spawned from
...
- Ship the binary at `WorkspaceAlberta.app/Contents/Resources/cua-driver`,
...
{ "mcpServers": { "computer": {
    ...
    "env": { "CUA_DRIVER_EMBEDDED": "1", "CUA_DRIVER_HOST_BUNDLE_ID": "ca.workspacealberta.app" }
} } }
```
